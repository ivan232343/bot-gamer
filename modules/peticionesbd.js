...
module.exports = {
    ...
    sp_register_interaction_doc: async (query = { interaction, dni }) => {
        const peticion = await get(`call \`sp_register_interaction-doc\`('${query.interaction}','${query.dni}')`)
    ...
    sp_update_ticket_atention: async (query = { interaction, currentid }) => {
        const peticion = await get(`call \`sp_update_ticket-atention\`('${query.interaction}','${query.currentid}')`)
    ...
    sp_validate_serv_gamer: async (query) => {
        const peticion = await get(`CALL \`sp_validate_serv-gamer\`('${query}')`)
    ...
    sp_validate_gamer_to_init: async (query = { doc, namecl, planPicked }) => {
        const peticion = await get(`CALL \`sp_validate_gamer-to-init\`('${query.namecl}','${query.doc}',${query.planPicked})`)
